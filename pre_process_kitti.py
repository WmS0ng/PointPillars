...
def create_data_info_pkl(data_root, data_type, prefix, label=True, db=False):
    ...
    ids_file = os.path.join(
        CUR, "pointpillars", "dataset", "ImageSets", f"{data_type}.txt"
    )
            ...
            if db:
                ...
                for j in range(n_valid_bbox):
                    ...
                    db_points_saved_name = os.path.join(
                        db_points_saved_path, f"{int(id)}_{name[j]}_{j}.bin"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prefix", default="kitti", help="the prefix name for the saved .pkl file"
    )
